...
def test_release_artifact_template_pack_exists_with_required_files() -> None:
    required_paths = [
        ...
        OPS_EVIDENCE_DIR / "valdrics_disposition_register_TEMPLATE.json",
...
def test_release_artifact_template_pack_contains_required_contract_tokens() -> None:
    ...
    assert "valdrics_disposition_register_YYYY-MM-DD.json" in readme_raw
    ...
    assert '"finding_id"' in valdrics_disposition_raw
    assert '"review_by"' in valdrics_disposition_raw
    assert '"exit_criteria"' in valdrics_disposition_raw
