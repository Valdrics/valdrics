...
def _set_strict_env() -> None:
    ...
    os.environ["ENFORCEMENT_EXPORT_SIGNING_SECRET"] = (
        "ci-enforcement-export-signing-secret-32-char"
    )
...
def test_validate_runtime_env_loads_explicit_env_file(
    tmp_path: Path, monkeypatch, capsys
) -> None:
...
def test_validate_runtime_env_rejects_directory_env_file(
    ...
    result = validate_runtime_env.main(["--env-file", str(env_dir)])

    assert result == 1
    assert "env-file must be a file path" in capsys.readouterr().err


def test_validate_runtime_env_restores_environment_after_validation(
    tmp_path: Path, capsys
) -> None:
    env_file = tmp_path / "production.env"
    env_file.write_text(
    ...
    with patch.dict(os.environ, original_environment, clear=True):
        with patch(
            ...
            result = validate_runtime_env.main(
                ["--environment", "production", "--env-file", str(env_file)]
            )
