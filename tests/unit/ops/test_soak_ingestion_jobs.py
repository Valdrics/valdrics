...
import scripts.soak_ingestion_jobs as soak_script
...
def test_resolve_output_path_rejects_relative_repo_escape() -> None:
    with pytest.raises(ValueError, match="out must stay within repo root when relative"):
        soak_script._resolve_output_path("../outside.json")
...
def test_resolve_output_path_rejects_directory(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="out must be a file path"):
        soak_script._resolve_output_path(str(output_dir))
...
def test_write_output_stages_before_promotion(
    ...
    staged_path = tmp_path / ".soak.json.tmp"
    ...
    monkeypatch.setattr(soak_script, "stage_json_file", _fake_stage)
    monkeypatch.setattr(soak_script, "promote_staged_file", _fake_promote)

    soak_script._write_output(output_path, {"ok": True})
