...
def test_verify_managed_release_readiness_prefers_runtime_report_public_values(
    ...
    runtime_payload["resolved_public_runtime_values"]["FRONTEND_URL"] = "https://report.example.com"
...
def test_verify_managed_release_readiness_reports_dashboard_gate_exception(
    ...
    assert "dashboard runtime contract verification failed unexpectedly: port bind denied" in errors
...
def test_verify_managed_release_readiness_reports_audit_gate_failures(tmp_path: Path) -> None:
    runtime_report, migration_report, deployment_report = _seed_reports(tmp_path)
