...
@pytest.mark.asyncio
async def test_create_azure_connection_denied_on_free_tier(
...
async def test_create_azure_connection_allowed_on_pro_tier(
    ...
    result = await db_session.execute(
        select(AzureConnection).where(AzureConnection.subscription_id == sub_id)
    )
...
async def test_verify_aws_connection(async_client: AsyncClient, db_session, mock_user):
    ...
    with patch(
        ...
        response = await async_client.post(
            f"/api/v1/settings/connections/aws/{conn.id}/verify"
        )
        assert response.status_code == 200
