...
async def test_carbon_settings_lifecycle(
    ...
    mock_user = CurrentUser(
        id=user_id, tenant_id=tenant_id, email="test@carbon.io", role=UserRole.ADMIN
    )
...
@pytest.mark.asyncio
async def test_notifications_settings_lifecycle(
    async_client: AsyncClient, db, mock_user_id, mock_tenant_id, app
):
...
@pytest.mark.asyncio
async def test_connections_settings_lifecycle(
    async_client: AsyncClient, db, mock_user_id, mock_tenant_id, app
):
    ...
    tenant_id = uuid.UUID(str(mock_tenant_id))
    mock_user = CurrentUser(
        id=user_id, tenant_id=tenant_id, email="test@conns.io", role=UserRole.ADMIN
    )
    app.dependency_overrides[get_current_user] = lambda: mock_user
