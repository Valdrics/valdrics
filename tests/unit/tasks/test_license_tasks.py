...
@pytest.mark.asyncio
async def test_license_governance_sweep_caps_tenant_scope_before_enqueue() -> None:
    ...
    with (
        patch("app.tasks.license_tasks._open_db_session", return_value=_db_cm()),
        patch("app.tasks.license_tasks.BACKGROUND_JOBS_ENQUEUED"),
        patch("app.tasks.license_tasks.logger") as mock_logger,
        patch(
            "app.tasks.license_tasks.get_settings",
            return_value=MagicMock(SCHEDULER_SYSTEM_SWEEP_MAX_TENANTS=2),
        ),
    ):
        await _license_governance_sweep_logic()
