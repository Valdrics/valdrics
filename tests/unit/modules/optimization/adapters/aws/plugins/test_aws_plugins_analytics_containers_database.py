...
async def test_idle_sagemaker_cur_records():
    ...
    with patch(
        ...
        mock_analyzer.return_value.find_idle_sagemaker_endpoints.return_value = [
            {"resource_id": "ep-1"}
        ]
...
async def test_idle_sagemaker_cloudwatch(monkeypatch):
    ...
    with patch(
        ...
        assert zombies[0]["resource_id"] == "ep-1"
        assert zombies[0]["pricing_evidence"]["pricing_confidence"] == (
...
async def test_stale_ecr_images_detected(monkeypatch):
    ...
    monkeypatch.setattr(
        plugin, "_get_client", lambda *args, **kwargs: AsyncContext(ecr)
    )
...
async def test_idle_rds_cloudwatch(monkeypatch):
    ...
    cloudwatch.get_metric_data = AsyncMock(
        return_value={"MetricDataResults": [{"Id": "m0", "Values": [0]}]}
    )
