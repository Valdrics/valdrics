...
class TestIdempotencyKeyGeneration:
    ...
    def test_generate_idempotency_key(self, webhook_service):
        ...
        key = webhook_service._generate_idempotency_key(
            provider="paystack", event_type="charge.success", reference="txn_123"
        )
    ...
    def test_idempotency_key_deterministic(self, webhook_service):
        ...
        key1 = webhook_service._generate_idempotency_key(
            "paystack", "charge.success", "txn_123"
        )
    ...
    def test_idempotency_key_different_for_different_inputs(self, webhook_service):
        ...
        key2 = webhook_service._generate_idempotency_key(
            "paystack", "charge.success", "txn_456"
        )
        key3 = webhook_service._generate_idempotency_key(
            "paystack", "invoice.create", "txn_123"
        )
    ...
    def test_idempotency_key_provider_matters(self, webhook_service):
        ...
        key_paystack = webhook_service._generate_idempotency_key(
            "paystack", "charge.success", "txn_123"
        )
        key_stripe = webhook_service._generate_idempotency_key(
            "stripe", "charge.success", "txn_123"
        )
...
class TestDuplicateDetection:
    ...
    @pytest.mark.asyncio
    async def test_is_duplicate_queries_webhook_retry_type(
...
class TestWebhookStorage:
    ...
    @pytest.mark.asyncio
    async def test_store_webhook_new(
...
class TestPendingWebhooks:
    ...
    @pytest.mark.asyncio
    async def test_get_pending_webhooks_filtered_by_provider(
        self, mock_db, webhook_service
    ):
...
class TestWebhookPayloadValidation:
    ...
    async def test_store_webhook_with_empty_payload(self, mock_db, webhook_service):
        ...
        with patch(
            ...
            await webhook_service.store_webhook(
                provider="paystack", event_type="charge.success", payload=empty_payload
            )
...
class TestProcessPaystackWebhook:
    ...
    @pytest.mark.asyncio
    async def test_process_paystack_webhook_missing_signature_material_fails_closed_in_production(
        self, mock_db
    ):
        mock_job = MagicMock(spec=BackgroundJob)
        mock_job.id = uuid.uuid4()
