...
class TestCarbonAwareSchedulerInitialization:
    ...
    def test_scheduler_init_with_both_api_keys(self):
        ...
        scheduler = CarbonAwareScheduler(
            wattime_key="wt-key", electricitymaps_key="em-key"
        )
...
class TestLowestCarbonRegionSelection:
    ...
    def test_get_avg_intensity_calculation(self):
        ...
        expected_avg = (
            profile.carbon_intensity_low + profile.carbon_intensity_high
        ) / 2
...
class TestOptimalExecutionTime:
    ...
    async def test_get_optimal_execution_time_region_with_best_hours(self):
        ...
        scheduler = CarbonAwareScheduler()
        optimal_time = await scheduler.get_optimal_execution_time(
            "eu-north-1", max_delay_hours=24
        )
    ...
    async def test_get_optimal_execution_time_unknown_region_returns_none(self):
        ...
        scheduler = CarbonAwareScheduler()
        optimal_time = await scheduler.get_optimal_execution_time(
            "unknown-xyz", max_delay_hours=24
        )
...
class TestWorkloadDeferralLogic:
    ...
    async def test_should_defer_critical_workload_never(self):
        ...
        should_defer = await scheduler.should_defer_workload(
            "ap-south-1", workload_type="critical"
        )
    ...
    async def test_should_defer_batch_workload_high_intensity(self):
        ...
        should_defer = await scheduler.should_defer_workload(
            "ap-south-1", workload_type="batch"
        )
    ...
    async def test_should_defer_batch_workload_low_intensity(self):
        ...
        should_defer = await scheduler.should_defer_workload(
            "eu-north-1", workload_type="batch"
        )
    ...
    async def test_should_defer_standard_workload_very_high_only(self):
        ...
        low_carbon_defer = await scheduler.should_defer_workload(
            "eu-north-1", workload_type="standard"
        )
...
class TestCarbonSavingsEstimation:
    ...
    def test_estimate_carbon_savings_identical_regions(self):
        ...
        savings = scheduler.estimate_carbon_savings(
            region_from="us-east-1", region_to="us-east-1", compute_hours=1000
        )
    ...
    def test_estimate_carbon_savings_with_unknown_regions(self):
        ...
        savings = scheduler.estimate_carbon_savings(
            region_from="unknown-1", region_to="unknown-2", compute_hours=100
        )
...
class TestCarbonSchedulerWrapAround:
    ...
    async def test_get_optimal_execution_time_wrap_around_midnight(self):
        ...
        with patch(
            "app.modules.reporting.domain.carbon_scheduler.datetime"
        ) as mock_datetime:
...
class TestFreshnessEnforcement:
    ...
    async def test_freshness_enforced_on_intensity_check(self):
        ...
        with patch(
            "app.modules.reporting.domain.carbon_scheduler.validate_carbon_data_freshness"
        ) as mock_val:
