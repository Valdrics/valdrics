    ...
    async def test_create_checkout_session_success(self, mock_db, mock_settings):
        ...
        with patch(
            ...
            mock_client_instance.initialize_transaction = AsyncMock(
                return_value={
                    "data": {"authorization_url": "http://pay", "reference": "ref1"}
                }
            )
...
class TestWebhookHandler:
    ...
    async def test_handle_subscription_disable(self, mock_db, mock_settings):
        handler = WebhookHandler(mock_db)
        payload = json.dumps(
            {"event": "subscription.disable", "data": {"subscription_code": "SUB_123"}}
        ).encode()
        ...
        payload = json.dumps(
            {
                ...
                "data": {"subscription_code": "SUB_123", "invoice_code": "INV_123"},
            }
        ).encode()
