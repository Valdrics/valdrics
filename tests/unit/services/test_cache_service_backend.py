"""Tests for CacheService against an injected async cache backend."""
...
@pytest.fixture
def cache_service(mock_cache_backend):
    with patch(
...
@pytest.mark.asyncio
async def test_get_analysis_hit(cache_service, mock_cache_backend):
    """Test cache hit for analysis."""
    tenant_id = uuid4()
    mock_cache_backend.get.return_value = json.dumps({"result": "good"})

    res = await cache_service.get_analysis(tenant_id)
    assert res == {"result": "good"}
    mock_cache_backend.get.assert_called_with(f"analysis:{tenant_id}")


@pytest.mark.asyncio
async def test_get_analysis_miss(cache_service, mock_cache_backend):
...
@pytest.mark.asyncio
async def test_set_analysis(cache_service, mock_cache_backend):
...
@pytest.mark.asyncio
async def test_get_cost_data(cache_service, mock_cache_backend):
    """Test getting cached cost data."""
    tenant_id = uuid4()
    date_range = "2026-01"
    mock_cache_backend.get.return_value = json.dumps([{"c": 10}])
...
@pytest.mark.asyncio
async def test_set_cost_data(cache_service, mock_cache_backend):
...
@pytest.mark.asyncio
async def test_invalidate_tenant(cache_service, mock_cache_backend):
    """Test child invalidation."""
    tenant_id = uuid4()
    await cache_service.invalidate_tenant(tenant_id)
    mock_cache_backend.delete.assert_called_with(f"analysis:{tenant_id}")
