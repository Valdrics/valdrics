...
class TestZombieServiceEdgeCases:
    ...
    async def test_scan_for_tenant_connection_failure(self, zombie_service, mock_db):
        ...
        with (
            patch("app.modules.optimization.domain.service.select"),
            patch(
    ...
    async def test_scan_for_tenant_partial_failure(self, zombie_service, mock_db):
        ...
        with (
            ...
            mock_aws_detector.scan_all.return_value = {
                "idle_instances": [{"id": "i-123", "owner": "me"}]
            }
    ...
    @pytest.mark.asyncio
    async def test_scan_with_on_category_complete_callback(
        self, zombie_service, mock_db
    ):
        ...
        with (
            ...
            await zombie_service.scan_for_tenant(
                tenant_id, on_category_complete=mock_callback
            )
        ...
        with patch(
            ...
            with patch("app.shared.core.notifications.logger"):
                await NotificationDispatcher.notify_zombies(
                    large_zombies, estimated_savings=50000.0
                )

                # Should handle large payload without issues
                mock_slack.notify_zombies.assert_called_once_with(
                    large_zombies, 50000.0
                )
    ...
    async def test_cross_tenant_data_leak_prevention(self):
        ...
        with patch("app.shared.db.session.settings") as mock_settings:
            ...
            with pytest.raises(Exception) as exc_info:
                check_rls_policy(
                    mock_conn, None, "SELECT * FROM cost_records", {}, {}, False
                )
