...
async def test_data(db):
    ...
    user = User(
        id=uuid4(), tenant_id=tenant.id, email="test@example.com", role=UserRole.ADMIN
    )
    db.add(tenant)
    db.add(user)
    await db.commit()
...
async def test_create_checkout_success(ac: AsyncClient, test_data, db):
    ...
    with (
        ...
        payload = {"tier": "growth", "billing_cycle": "monthly"}

        response = await ac.post(
            "/api/v1/billing/checkout", json=payload, headers=headers
        )
...
async def test_webhook_charge_success_activates_subscription(
    ...
    webhook_payload = {
        ...
        "data": {
            ...
            "status": "success",
            "amount": 290000,
            "metadata": {"tenant_id": str(test_data["tenant"].id), "tier": "growth"},
            "customer": {"customer_code": "CUS_test_001", "email": "test@example.com"},
            "authorization": {
                "authorization_code": "AUTH_test_999",
                "card_type": "visa",
            },
            "plan": {"name": "Growth Plan", "plan_code": "PLN_growth_xxx"},
        },
    }
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        result = await db.execute(
            select(TenantSubscription).where(
                TenantSubscription.tenant_id == test_data["tenant"].id
            )
        )
