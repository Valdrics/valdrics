    ...
    **kwargs: Any,
        ...
        try:
            async with self._get_client(
                session, "eks", region, credentials, config=config
            ) as eks:
                ...
                async for page in paginator.paginate():
                    for cluster_name in page.get("clusters", []):
                        try:
                            ...
                            total_nodes = 0
                            async for ng_page in ng_paginator.paginate(
...
class IdleElastiCachePlugin(ZombiePlugin):
    ...
    **kwargs: Any,
        ...
        try:
            async with self._get_client(
                session, "elasticache", region, credentials, config=config
            ) as elasticache:
                async with self._get_client(
                    ...
                    async for page in paginator.paginate(ShowCacheNodeInfo=True):
                        for cluster in page.get("CacheClusters", []):
                            ...
                            try:
                                ...
                                avg_cpu = (
                                    datapoints[0].get("Average", 0) if datapoints else 0
                                )
                                ...
                                logger.warning(
                                    "elasticache_metric_fetch_failed",
                                    cluster=cluster_id,
                                    error=str(e),
                                )
                ...
                async for page in paginator.paginate():
                    for notebook in page.get("NotebookInstances", []):
                        ...
                        try:
                            ...
                            logger.warning(
                                "sagemaker_notebook_check_failed",
                                notebook=name,
                                error=str(e),
                            )
