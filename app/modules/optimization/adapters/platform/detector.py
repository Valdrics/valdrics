...
import app.modules.optimization.adapters.platform.plugins.core  # noqa: F401

logger = structlog.get_logger()


class PlatformZombieDetector(BaseZombieDetector):
    """
    Zombie detector for internal platform/shared-services spend feeds.
    """
    ...
    def _initialize_plugins(self) -> None:
        plugins = registry.get_plugins_for_provider("platform")
        self.plugins = [
        ...
        if skipped:
            logger.warning(
                "platform_detector_skipping_noncanonical_plugins", categories=skipped
            )
    ...
    async def _execute_plugin_scan(self, plugin: ZombiePlugin) -> List[Dict[str, Any]]:
        ...
        if not isinstance(results, list):
            logger.warning(
                "platform_plugin_scan_invalid_result",
