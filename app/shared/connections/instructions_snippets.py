...
def build_azure_setup_snippet(tenant_id: str, issuer: str) -> dict[str, str]:
    snippet = (
        "# 1. Create App Registration in Azure AD\n"
        "# 2. Create a Federated Credential with these details:\n"
        f"Issuer: {issuer} (IMPORTANT: Must be publicly reachable by Azure)\n"
        f"Subject: tenant:{tenant_id}\n"
        "Audience: api://AzureADTokenExchange\n"
        "\n# Or run this via Azure CLI:\n"
        "az ad app federated-credential create --id <YOUR_CLIENT_ID> "
...
def build_gcp_setup_snippet(tenant_id: str, issuer: str) -> dict[str, str]:
    snippet = (
        "# Run this to create an Identity Pool and Provider for Valdrics\n"
        f"# IMPORTANT: Your Valdrics instance must be reachable at {issuer}\n"
        'gcloud iam workload-identity-pools create "valdrics-pool" --location="global" --display-name="Valdrics Pool"\n'
        'gcloud iam workload-identity-pools providers create-oidc "valdrics-provider" '
        '--location="global" --workload-identity-pool="valdrics-pool" '
        f'--issuer-uri="{issuer}" '
...
def build_saas_setup_snippet(
    tenant_id: str,
    api_url: str,
    saas_catalog: list[dict[str, object]],
) -> dict[str, object]:
...
def build_license_setup_snippet(
    tenant_id: str,
    api_url: str,
    license_catalog: list[dict[str, object]],
) -> dict[str, object]:
    native_payload = (
        "{\n"
...
def build_platform_setup_snippet(tenant_id: str, api_url: str) -> dict[str, object]:
    ledger_payload = (
        "{\n"
...
def build_hybrid_setup_snippet(tenant_id: str, api_url: str) -> dict[str, object]:
    ledger_payload = (
        "{\n"
