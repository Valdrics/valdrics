...
class DeltaAnalysisResult:
    ...
    def has_significant_changes(self) -> bool:
        ...
        return (
            len(self.top_increases) > 0
            or len(self.top_decreases) > 0
            or len(self.new_resources) > 0
            or abs(self.total_change_percent) >= SIGNIFICANT_CHANGE_PERCENT
        )
...
class DeltaAnalysisService:
    ...
    async def compute_delta(
        ...
        current_by_resource = self._aggregate_by_resource(
            current_costs, days_to_compare
        )
        previous_by_resource = (
            self._aggregate_by_resource(previous_costs, days_to_compare)
            if previous_costs
            else {}
        )
        ...
        result.total_current = sum(
            r["daily_cost"] for r in current_by_resource.values()
        )
        result.total_previous = sum(
            r["daily_cost"] for r in previous_by_resource.values()
        )
        result.total_change = result.total_current - result.total_previous
        if result.total_previous > 0:
            result.total_change_percent = (
                result.total_change / result.total_previous
            ) * 100

        # Find deltas for each resource
        all_deltas: List[CostDelta] = []
        all_resource_ids = set(current_by_resource.keys()) | set(
            previous_by_resource.keys()
        )
        ...
        result.top_increases = sorted(
            increases, key=lambda x: x.change_amount, reverse=True
        )[:TOP_MOVERS_COUNT]

        result.top_decreases = sorted(decreases, key=lambda x: x.change_amount)[
            :TOP_MOVERS_COUNT
        ]
    ...
    def _aggregate_by_resource(
        self, costs: Optional[List[Dict[str, Any]]], days: int
            ...
            for group in groups:
                ...
                if len(keys) >= 1:
                    ...
                    amount = float(
                        ...
                        or metrics.get("BlendedCost", {}).get("Amount", 0)
                        or 0
                    )
...
async def analyze_with_delta(
    ...
    delta = await delta_service.compute_delta(
        tenant_id=tenant_id, current_costs=current_costs, previous_costs=previous_costs
    )

    # If no significant changes, return a simple response
    if not delta.has_significant_changes:
        logger.info("delta_no_significant_changes", tenant_id=str(tenant_id))
        result = {
