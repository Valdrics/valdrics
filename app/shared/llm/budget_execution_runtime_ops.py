"""Runtime budget execution operations for reservation, usage, and alerts."""
...
async def check_and_reserve_budget(
    *,
    ...
    actor_type: str,
    client_ip: str | None,
    normalize_actor_type_fn: Callable[[Any], str],
    record_authenticated_abuse_signal_fn: Any,
    enforce_global_abuse_guard_fn: Any,
    enforce_fair_use_guards_fn: Any,
    execution_recoverable_errors: tuple[type[Exception], ...],
) -> Decimal:
    """Check tenant budget and reserve estimated cost atomically."""
    import app.shared.llm.budget_manager as manager_module

    estimated_cost = cast(
        Decimal,
        manager_cls.estimate_cost(prompt_tokens, completion_tokens, model, provider),
    )
    concurrency_slot_acquired = False
    tier_for_metrics = "unknown"
    normalized_actor_type = normalize_actor_type_fn(actor_type)
    ...
    try:
        ...
        await record_authenticated_abuse_signal_fn(
        ...
        await enforce_global_abuse_guard_fn(
            manager_cls=manager_cls,
            tenant_id=tenant_id,
            db=db,
            tier=tier,
        ...
        if manager_module.get_settings().LLM_FAIR_USE_GUARDS_ENABLED:
            concurrency_slot_acquired = await enforce_fair_use_guards_fn(
                manager_cls=manager_cls,
                tenant_id=tenant_id,
                db=db,
                tier=tier,
        ...
        if not budget:
            ...
            if tier in {manager_module.PricingTier.STARTER, manager_module.PricingTier.GROWTH}:
                default_limit = Decimal("10.00")
            elif tier in {manager_module.PricingTier.PRO, manager_module.PricingTier.ENTERPRISE}:
        ...
        if budget.budget_reset_at.year != now.year or budget.budget_reset_at.month != now.month:
            manager_module.logger.info(
        ...
        if estimated_cost > remaining_budget:
            ...
            raise manager_module.BudgetExceededError(
                (
                    "LLM budget exceeded. Required: "
                    f"${float(estimated_cost):.4f}, Available: ${float(remaining_budget):.4f}"
                ),
                details={
                    "monthly_limit": float(limit),
                    "current_total_committed": float(current_total),
        ...
        raise
    except execution_recoverable_errors as exc:
        if concurrency_slot_acquired:
            await manager_cls._release_fair_use_inflight_slot(tenant_id)
...
async def record_usage_entry(
    *,
    ...
    actor_type: str,
    normalize_actor_type_fn: Callable[[Any], str],
    compose_request_type_fn: Callable[[str, str], str],
    coerce_decimal_fn: Callable[[Any], Decimal | None],
    metric_recoverable_errors: tuple[type[Exception], ...],
    rollback_recoverable_errors: tuple[type[Exception], ...],
    execution_recoverable_errors: tuple[type[Exception], ...],
) -> None:
    """Record actual LLM usage and update budget counters/alerts."""
    import app.shared.llm.budget_manager as manager_module
    ...
    try:
        ...
        if budget:
            ...
            pending = coerce_decimal_fn(pending_raw)
            spend = coerce_decimal_fn(spend_raw)
        ...
        usage = manager_module.LLMUsage(
            ...
            is_byok=is_byok,
            operation_id=operation_id,
            request_type=compose_request_type_fn(normalized_actor_type, request_type),
        )
        db.add(usage)
        ...
        except metric_recoverable_errors as exc:
        ...
        if callable(rollback_fn):
            ...
            except rollback_recoverable_errors as rollback_exc:
                manager_module.logger.warning(
...
async def check_budget_state(
    *,
    ...
    db: AsyncSession,
    coerce_decimal_fn: Callable[[Any], Decimal | None],
    coerce_threshold_percent_fn: Callable[[Any], Decimal],
    coerce_bool_fn: Callable[[Any], bool],
    cache_recoverable_errors: tuple[type[Exception], ...],
) -> Any:
    """Unified budget state computation for tenants."""
    import app.shared.llm.budget_manager as manager_module
    ...
    limit_dec = coerce_decimal_fn(getattr(budget, "monthly_limit_usd", None))
    spend_dec = coerce_decimal_fn(getattr(budget, "monthly_spend_usd", Decimal("0")))
    pending_dec = coerce_decimal_fn(getattr(budget, "pending_reservations_usd", Decimal("0")))
    if limit_dec is None or spend_dec is None or pending_dec is None:
        return manager_module.BudgetStatus.OK

    limit = manager_cls._to_decimal(limit_dec)
    current_usage = manager_cls._to_decimal(spend_dec) + manager_cls._to_decimal(pending_dec)
    threshold = (
        coerce_threshold_percent_fn(getattr(budget, "alert_threshold_percent", Decimal("80")))
        / Decimal("100")
    )
    hard_limit = coerce_bool_fn(getattr(budget, "hard_limit", False))
...
async def check_budget_and_alert(
    *,
    ...
    coerce_decimal_fn: Callable[[Any], Decimal | None],
    coerce_threshold_percent_fn: Callable[[Any], Decimal],
    alert_recoverable_errors: tuple[type[Exception], ...],
) -> None:
    """Check budget threshold and dispatch warning/critical alerts."""
    import app.shared.llm.budget_manager as manager_module
    ...
    limit = manager_cls._to_decimal(limit_dec)
    threshold_percent = coerce_threshold_percent_fn(
    ...
    if usage_percent >= threshold_percent and not already_sent:
        ...
        except alert_recoverable_errors as exc:
            manager_module.logger.warning(
