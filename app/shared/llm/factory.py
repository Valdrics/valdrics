...
class LLMFactory:
    ...
    def validate_api_key(provider: str, api_key: Optional[str]) -> None:
        ...
        if not api_key:
            raise ValueError(
                f"LLM API key for provider '{provider}' is not configured."
            )
        ...
        if "xxx" in api_key.lower() or "change-me" in api_key.lower():
            ...
            raise ValueError(
                f"LLM API key for '{provider}' contains a placeholder. Use a real key."
            )
    ...
    @staticmethod
    def select_provider(
        input_text: str, tenant_byok_provider: Optional[str] = None
        ...
        if tenant_byok_provider:
            logger.info("llm_provider_byok", provider=tenant_byok_provider)
    ...
    def create_smart(
        ...
        provider, complexity = LLMFactory.select_provider(
            input_text=input_text, tenant_byok_provider=tenant_byok_provider
        )

        llm = LLMFactory.create(
            provider=provider, api_key=tenant_byok_key if tenant_byok_provider else None
        )
