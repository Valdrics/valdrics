...
from app.shared.adapters.saas_native_stream_ops import (
    ...
    stream_salesforce_cost_and_usage as _stream_salesforce_cost_and_usage_impl,
    stream_stripe_cost_and_usage as _stream_stripe_cost_and_usage_impl,
)
...
class SaaSAdapter(BaseAdapter):
    ...
    async def _stream_stripe_cost_and_usage(
        ...
        async for row in _stream_stripe_cost_and_usage_impl(
            adapter=self,
            start_date=start_date,
            end_date=end_date,
            logger=logger,
            convert_to_usd_fn=convert_to_usd,
            as_float_fn=as_float,
            parse_timestamp_fn=parse_timestamp,
            currency_conversion_recoverable_errors=SAAS_CURRENCY_CONVERSION_RECOVERABLE_ERRORS,
        ):
            yield row
    ...
    async def _stream_salesforce_cost_and_usage(
        ...
        async for row in _stream_salesforce_cost_and_usage_impl(
            adapter=self,
            start_date=start_date,
            end_date=end_date,
            logger=logger,
    ...
    async def _get_json(
        ...
        *,
        ...
        return await _get_json_impl(
            url=url,
            headers=headers,
            params=params,
            request_fn=lambda endpoint, request_headers, request_params: _saas_get_request(
                url=endpoint,
                headers=request_headers,
                params=request_params,
            ),
            execute_with_http_retry_fn=execute_with_http_retry,
    ...
    async def get_resource_usage(
        ...
        return project_cost_rows_to_resource_usage(
            ...
            resource_id=resource_id,
            default_provider="saas",
            default_source_adapter="saas_cost_feed",
        )
