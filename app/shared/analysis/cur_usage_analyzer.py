...
class CURUsageAnalyzer:
    ...
    def find_low_usage_instances(self, days: int = 14) -> List[Dict[str, Any]]:
        ...
        for resource_id, data in instance_usage.items():
            usage_ratio = (
                float(data["total_usage_hours"]) / expected_hours
                if expected_hours > 0
                else 0
            )
        ...
        logger.info(
            "cur_ec2_analysis_complete",
            analyzed=len(instance_usage),
            idle=len(low_usage_instances),
        )
    ...
    def find_idle_rds_databases(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        logger.info(
            "cur_rds_analysis_complete",
            analyzed=len(rds_usage),
            idle=len(idle_databases),
        )
    ...
    def find_idle_redshift_clusters(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        logger.info(
            "cur_redshift_analysis_complete",
            analyzed=len(redshift_usage),
            idle=len(idle_clusters),
        )
    ...
    def find_idle_nat_gateways(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        logger.info(
            "cur_nat_analysis_complete", analyzed=len(nat_usage), idle=len(idle_nats)
        )
        return idle_nats
    ...
    def find_idle_sagemaker_endpoints(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        logger.info(
            "cur_sagemaker_analysis_complete",
            analyzed=len(sagemaker_usage),
            idle=len(idle_endpoints),
        )
    ...
    def find_idle_elasticache_clusters(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        logger.info(
            "cur_elasticache_analysis_complete",
            analyzed=len(cache_usage),
            idle=len(idle_clusters),
        )
        return idle_clusters
