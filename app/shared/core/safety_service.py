...
class SafetyGuardrailService:
    ...
    async def _check_global_kill_switch(
        self, tenant_id: UUID, estimated_impact: Decimal
    ) -> None:
    ...
    async def _check_monthly_hard_cap(self, tenant_id: UUID) -> None:
        ...
        summary = await CostAggregator.get_summary(
            self.db, tenant_id, start_date=start_of_month, end_date=today
        )

        current_spend = (
            summary.total_cost if hasattr(summary, "total_cost") else Decimal("0")
        )
        cap = Decimal(str(settings.monthly_hard_cap_usd))

        if current_spend > cap:
            logger.warning(
                "budget_hard_cap_breached",
                tenant_id=str(tenant_id),
            ...
            percent_used = (
                (float(current_spend) / float(cap) * 100.0) if cap > 0 else 100.0
            )
    ...
    async def _check_circuit_breaker(self, tenant_id: UUID) -> None:
        ...
        if failure_count >= 5:  # Threshold of 5 failures per day
            logger.warning(
                "remediation_circuit_breaker_open",
                tenant_id=str(tenant_id),
