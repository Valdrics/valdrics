...
class StrategyType(str, Enum):
    ...
    CUD = "committed_use_discount"
    AZURE_RESERVATION = "azure_reservation"
    SPOT = "spot_instance"
...
class StrategyRecommendation(Base):
    ...
    estimated_monthly_savings_low: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(12, 2), nullable=True
    )
    estimated_monthly_savings: Mapped[Decimal] = mapped_column(
        Numeric(12, 2), nullable=False
    )
    estimated_monthly_savings_high: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(12, 2), nullable=True
    )
    ...
    tenant: Mapped["Tenant"] = relationship(
        "Tenant"
    )  # Assuming Tenant model exists and is importable via string
