    ...
    async with session_factory() as db:
        for provider, models in LLM_PRICING.items():
            for model_name, cost in models.items():
                ...
                free_tokens = (
                    cost.get("free_tier_tokens", 0)
                    if isinstance(cost, dict)
                    else cost.free_tier_tokens
                )
...
async def update_pricing(
    provider: str, model: str, input_cost: float, output_cost: float, free_tokens: int
):
    """Update or create pricing for a specific provider/model."""
    session_factory = await get_db_session_factory()
    async with session_factory() as db:
        stmt = select(LLMProviderPricing).where(
            LLMProviderPricing.provider == provider, LLMProviderPricing.model == model
        )
        result = await db.execute(stmt)
        existing = result.scalar_one_or_none()
...
def main(argv: list[str] | None = None) -> int:
    ...
    update_parser = subparsers.add_parser(
        "update", help="Update specific model pricing"
    )
    update_parser.add_argument("--provider", required=True)
    update_parser.add_argument("--model", required=True)
    update_parser.add_argument(
        "--input", type=float, required=True, help="Cost per 1M input tokens"
    )
    update_parser.add_argument(
        "--output", type=float, required=True, help="Cost per 1M output tokens"
    )
