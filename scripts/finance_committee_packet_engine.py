"""Computation engine for finance guardrails and committee packet outputs."""
...
from scripts.finance_committee_packet_common import (
    ...
    parse_non_empty_str,
    parse_thresholds,
    parse_tier_float_map,
    safe_margin_percent,
)
from scripts.finance_committee_packet_scenarios import compute_scenario_rows


def build_tier_unit_economics(
    *,
    ...
    subscriptions = index_by_tier(subscription_rows, field="telemetry.tier_subscription_snapshot")
    llm_usage = index_by_tier(llm_rows, field="telemetry.tier_llm_usage")
...
def compute_metrics(
    *,
    ...
    stress_margin = safe_margin_percent(
        total_effective_mrr,
...
def build_gate_results(
    *,
    ...
    if len(close_history) < required_consecutive:
        raise ValueError(
            "close_history must include at least " f"{required_consecutive} entries"
        )
...
def build_finance_outputs(
    *,
    telemetry: dict[str, Any],
    assumptions: dict[str, Any],
) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    annual_mix_by_tier = parse_tier_float_map(
        assumptions,
        field="annual_mix_by_tier",
        min_value=0.0,
        max_value=1.0,
    )
    infra_cogs_pct_by_tier = parse_tier_float_map(
        assumptions,
        field="infra_cogs_percent_of_effective_mrr_by_tier",
        min_value=0.0,
    )
    support_cogs_per_subscription_by_tier = parse_tier_float_map(
        assumptions,
        field="support_cogs_per_active_subscription_usd_by_tier",
        min_value=0.0,
    )
    support_cogs_per_dunning_event_usd = parse_float(
    ...
    conversion_signals = {
        "growth_to_pro_conversion_mom_delta_percent": parse_float(
        ...
        "pro_to_enterprise_conversion_mom_delta_percent": parse_float(
    ...
    stress_infra_multiplier = parse_float(
        stress_scenario_raw.get("infra_cost_multiplier"),
        field="stress_scenario.infra_cost_multiplier",
        min_value=1.0,
    )
    thresholds = parse_thresholds(assumptions)
    close_history = parse_close_history(assumptions)
    ...
    close_history.append(
        {
            "month": parse_non_empty_str(window.get("label"), field="telemetry.window.label"),
    ...
    finance_guardrails = {
        ...
        "window": {
            "start": parse_non_empty_str(window.get("start"), field="telemetry.window.start"),
            "end": parse_non_empty_str(window.get("end"), field="telemetry.window.end"),
            "label": parse_non_empty_str(window.get("label"), field="telemetry.window.label"),
        },
        "metrics": metrics,
        "thresholds": thresholds,
        "close_history": close_history,
