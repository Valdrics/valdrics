...
INTERACTIVE_CONFIRM_TOKEN = "ISSUE_VALDRICS_EMERGENCY_TOKEN"  # nosec B105 - interactive confirmation token
DEFAULT_TTL_HOURS = 1
MAX_TTL_HOURS = 4
MIN_REASON_LENGTH = 16
PROD_EMERGENCY_BYPASS = "I_UNDERSTAND_EMERGENCY_TOKEN_RISK"  # nosec B105 - operator bypass phrase
...
def _validate_request(
    *,
    ...
    if len(reason.strip()) < MIN_REASON_LENGTH:
        raise RuntimeError(f"--reason must be at least {MIN_REASON_LENGTH} characters.")
    if ttl_hours < 1 or ttl_hours > MAX_TTL_HOURS:
        raise RuntimeError(f"--ttl-hours must be between 1 and {MAX_TTL_HOURS}.")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate a short-lived emergency token."
    )
    parser.add_argument("--email", default="", help="Target user email.")
    parser.add_argument(
        "--operator", default="", help="Operator identifier (email or handle)."
    )
    parser.add_argument(
        "--reason",
        default="",
